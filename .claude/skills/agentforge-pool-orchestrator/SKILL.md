---
name: agentforge-pool-orchestrator
description: Search the AgentForge pool before non-trivial tasks; on miss use skill_find / skill_pull (and mcp/api/topic find), not bulk curate. Mandatory team_compose before design specs.
---

# AgentForge Pool Orchestrator

> The standing instruction lives in `CLAUDE.md` (see
> `CLAUDE-md-block.md` in this directory), because a skill description
> competes for attention against every other installed skill while
> `CLAUDE.md` is always in context. This file is the same instruction,
